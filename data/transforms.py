...
def fft2(data):
    ...
    data = torch.view_as_real(
        torch.fft.fft2(torch.view_as_complex(data), dim=(-2, -1), norm="ortho")
    )
    data = fftshift(data, dim=(-3, -2))
    return data
...
def ifft2_regular(data):
    ...
    data = ifftshift(data, dim=(-3, -2))
    data = torch.view_as_real(
        torch.fft.ifft2(torch.view_as_complex(data), dim=(-2, -1), norm="ortho")
    )
    data = fftshift(data, dim=(-3, -2))
...
def complex_abs(data):
    ...
    assert (
        data.size(-1) == 2
    )  # Make sure the last dimension is 2, meaning this tensor holds complex values
    return (data**2).sum(dim=-1).sqrt()  # every complex transform to real number
...
def normalize_instance_per_channel(data, eps=0.0):
    """
    Normalize the given tensor using:
        (data - mean) / (stddev + eps)
    where mean and stddev are computed from the data itself.
